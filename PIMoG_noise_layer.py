...
class ScreenShooting(nn.Module):
    ...
    def __init__(self):
        super(ScreenShooting, self).__init__()
        self.to_tensor = T.ToTensor()  # 用于将PIL图像转换为Tensor
        self.to_pil = T.ToPILImage()   # 用于将Tensor转换为PIL图像

    def forward(self, embed_image):
        # 如果输入是PIL图像，将其转换为Tensor
        if isinstance(embed_image, Image.Image):
            embed_image = self.to_tensor(embed_image).unsqueeze(0)  # 添加一个batch维度
        ...
        if isinstance(embed_image, Image.Image):
            noised_image = self.to_pil(noised_image.squeeze(0))  # 去掉batch维度并转换为PIL图像
